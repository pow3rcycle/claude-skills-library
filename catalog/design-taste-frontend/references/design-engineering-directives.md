# Design Engineering Directives (design-taste-frontend, Section 4)

> Split out of `SKILL.md` on 2026-08-12 to keep the skill body loadable. The SKILL.md keeps Section 4's heading, its non-negotiables, and a pointer here; the full directives live below, unchanged. `MASTER-DESIGN.md` wins on any conflict.
...
* **Sans font choice:**
  * **BANNED — zero exceptions:** `Inter`, and every member of its family (`Inter Tight`, `Inter Display`, `Inter Variable`). Not as a default, not as a fallback, not for "neutral / standard / Linear-style", not for public-sector or accessibility-first briefs. The old override is revoked. A neutral brief is served by `Geist` or `Plus Jakarta Sans`, which are just as legible and do not read as the default LLM face.
  * **Pick from:** `Geist`, `Satoshi`, `Cabinet Grotesk`, `Clash Display`, `Outfit`, `Plus Jakarta Sans`.
* **Pairings to know:** `Geist` + `Geist Mono`, `Satoshi` + `JetBrains Mono`, `Cabinet Grotesk` + `IBM Plex Mono`, `Plus Jakarta Sans` + `JetBrains Mono`.
...
* **SERIF DISCIPLINE (VERY DISCOURAGED AS DEFAULT):**
  ...
  * **Specifically BANNED, zero exceptions:** `Fraunces`, `Instrument_Serif`, `Playfair Display`, `Cormorant Garamond`. All four are training-data defaults and are read as an AI tell on sight.
  * **If a serif is justified** (rare, per the above), pick from this pool: **PP Editorial New, Tiempos Headline, Canela, Domaine Display, GT Sectra** — plus, when the brand supports them, Cardinal Grotesque, Reckless Neue, Recoleta, EB Garamond, IvyPresto, Migra, Editorial Old, Saol Display, Söhne Breit Kursiv, Schnyder, Tobias, NB Architekt, ITC Galliard.
...
* **No brief-based override.** A brief asking for "purple" does not unban the AI-purple aesthetic. If a brand file codifies purple / violet / lila, use that codified value - flat, as an accent or a solid field, never as a gradient backdrop or a glow. Purple gradients are a CRITICAL slop-lint finding regardless of what the brief said.
...
* **PREMIUM-CONSUMER PALETTE BAN (mandatory, second-most-recurring AI-tell):**
  ...
  * **Default alternatives (rotate, do not reuse):**
    ...
    - **Cobalt + Porcelain:** saturated blue against a single cool neutral (porcelain / cool off-white), no cream and no brass
  ...
  * **No brief-based override.** "Vintage", "artisan", "warm-craft", or a brief that names cream and brass does not unban the band - that is exactly the reflex being corrected. Get the warmth from texture, paper grain, photography, and type (a warm display face, letterpress-weight rules, generous ink), and keep the palette outside the band: rotate to one of the alternatives above. The listed hex families are usable only when a brand file already codifies them as the brand's own colors.
...
* **Bento Background Diversity (mandatory).** Bento and feature-grid sections cannot be 6 white-on-white cards with text inside. At least 2-3 cells in any multi-cell grid need real visual variation: a real image, a graphic pattern or texture, a data/chart mark, or a tinted surface one step off the canvas. **Not a gradient** — gradient cell backgrounds are banned here as everywhere; "add a gradient to the cell" is never the fix. A cream-on-cream bento with only typography inside reads as boring AI default, even when the rest of the page is good.
