...
Reference for `design-taste-frontend` Section 10. A lookup list of named patterns (hero paradigms, nav idioms, grids, scroll animations, micro-interactions) so the agent can reach for a specific named thing instead of re-deriving a generic one. Read when choosing a composition or when a brief names a pattern you want to confirm.
...
* **Mesh Gradient Background** - Organic lava-lamp blobs. BANNED as a backdrop (MASTER-DESIGN.md §2) — vocabulary entry kept only so the pattern can be named when auditing it OUT of a design.
