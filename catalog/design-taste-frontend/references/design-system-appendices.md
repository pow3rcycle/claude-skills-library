# Design-System Appendices — Source-Backed Reference

Reference for `design-taste-frontend` Section 2. Read when you have already chosen a design system and need its real install command, its canonical docs, or the honest Liquid Glass approximation.
...
> **Doctrine marker (read before copying).** The `linear-gradient` / `radial-gradient`
> layers below are the **sanctioned inset-highlight glass recipe** — they simulate the
> specular edge of a physical pane on a translucent element, which is why they are
> white-on-white at low alpha and confined to the border/`::before` highlight. They are
> **not** a licence for gradient card fills, gradient section backdrops, or a gradient
> anywhere else on the page. `MASTER-DESIGN.md` §2 still governs: outside this recipe,
> gradients are legal only inside illustration/photographic artwork, on one editorial
> accent word, or where a codified brand language file specifies them.
