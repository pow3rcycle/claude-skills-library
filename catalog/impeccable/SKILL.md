---
name: impeccable
description: Design-system craft skill for any frontend UI work — design, redesign, audit, polish, critique, harden, or animate websites, dashboards, product UI, and components; covers hierarchy, spacing, typography, color, motion, accessibility, UX copy, and design tokens. Not for backend-only tasks.
version: 3.9.1
user-invocable: true
...
allowed-tools:
  - Bash(npx impeccable *)
  - Bash(node "$HOME/.claude/skills/impeccable/scripts/*")
  - Bash(node $HOME/.claude/skills/impeccable/scripts/*)
...
6. **If the project is brand-new (no existing CSS tokens / theme / committed brand colors found in step 3)**, run `node "$HOME/.claude/skills/impeccable/scripts/palette.mjs"` (same home-path rule as step 1) to receive a brand seed color and composition guidance. This is the anchor for your primary brand color. Compose the rest of the palette (bg, surface, ink, accent, muted) around it per the script's instructions. Use OKLCH throughout. **Skip this step only if step 3 found committed brand colors in existing tokens; in that case identity-preservation wins.**
...
```bash
node "$HOME/.claude/skills/impeccable/scripts/pin.mjs" <pin|unpin> <command>
```
