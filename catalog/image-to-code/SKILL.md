---
name: image-to-code
description: For visually important web tasks: generate the design image(s) first, deeply analyze them, then implement the website to match as closely as possible — large section-specific images, clean spacious hero.
---
...
The full combinatorial variation engine — **Theme Paradigm, Background Character, Typography Character, Hero Architecture, Section System, Signature Component Set, Motion-Implied Language** — is shared with the sibling imagegen skills and lives in one place.
...
- shadow color mood
- image tint / grade
- gradient restraint or intensity (record what the image does; reproduce it only in the three homes doctrine §2 allows — otherwise implement the same value as a flat field)
...
The five-bucket taxonomy — **Layout slop, Visual slop, Typography slop, Content slop, Density slop** — is shared with the sibling imagegen skills.
