...
Section heading: `Designer Name - Perspective Handle` (hyphen, not an em-dash - the report is rendered text and the zero-em-dash rule applies to it). Lens take is the documented lens summary, NOT a quote from the person - render as `Through {Designer}'s lens` (apostrophe-s).
