...
description: Generate premium mobile app screen concepts and flows (iOS/Android) shown in subtle phone mockups — clean hierarchy, readable text, multi-screen consistency. Image generation only; writes no code.
---
...
- flat tinted surface steps (page → card +3–5% L)
- background-delta elevation instead of shadow
...
- bottom-to-top legibility scrim under the text block — legal because it is an **image treatment** (doctrine §2, legal home (a): it sits ON the photograph). Never a decorative gradient on a plain surface, and never a full-bleed brand wash behind a hero value.
...
These are the **mobile-specific** tells. The five shared buckets (Layout / Visual / Typography / Content / Density slop) are not repeated here — **read `~/.claude/skills/imagegen-shared/anti-slop-taxonomy.md` (installed) or the sibling `imagegen-shared/` folder in this repo** and apply both lists.
...
The axes below are **mobile-specific and authoritative for this skill** — do not substitute the web engine in `imagegen-shared/variation-engine.md`, which covers Hero Architecture, Section System and per-section Composition Anchors that do not apply to app screens.
...
3. barely-there flat surface wash (single tint step, no blend)
4. soft noise wash
...
2. brand-hue tinted neutral palette + sharp dark contrast (outside the cream/sand/beige + brass band)
3. cool mineral palette + clean highlight accent
4. editorial porcelain / charcoal / muted accent (cool-tinted, not the cream/sand/beige warm band)
...
- no gradient backdrops at all, brand washes behind a hero value included (doctrine §2: legal only inside illustration/photographic artwork, on one editorial accent word in a display heading, or where a codified brand language file specifies it) → a flat brand canvas or a real product visual
- no default purple-blue AI palettes → a committed single-hue palette named in the brief
