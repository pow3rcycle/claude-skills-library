---
name: gpt-taste
description: Elite UX/UI and advanced GSAP motion engineering — randomized layout variance, AIDA structure, wide editorial typography, gapless bento grids, strict ScrollTriggers, massive section spacing.
---

# CORE DIRECTIVE: AWWWARDS-LEVEL DESIGN ENGINEERING

> **BRAND register only — expressive marketing surfaces. Never apply to dashboards/app UI. Doctrine (`MASTER-DESIGN.md`) wins on any conflict.**
>
> This is a prescriptive page *generator* (GSAP + AIDA + bento), not a neutral taste gut-check. Everything below assumes a landing page, campaign site, or portfolio.
...
- **Marquee (earned, not default):** A continuously scrolling row is allowed **only** where the section genuinely benefits from it (a logo wall wider than the viewport, a ticker of live values) — it is never a filler for a section you could not fill. Not a default component: if a static row of logos fits, ship the static row. When used it must **pause on hover/focus**, stop entirely under `prefers-reduced-motion: reduce`, and never sit behind or beside body copy the user is reading.
...
- **Committed Backgrounds:** Section backgrounds are **flat, committed, hue-tinted surfaces** — never gradient backdrops. Pick a tinted neutral ramp (every step carries the same hue cast; dark grounds sit at 6–10% lightness, never `#000000`) and separate sections by a background *delta* of one or two steps, not by a mesh, aurora, orb, or radial wash. A flat committed surface is not "boring" — it is what every award-tier product ships; boring is the *un-committed* mid-grey `#f5f5f5`. Depth comes from real imagery, a fixed `pointer-events-none` grain layer, and typographic scale. **Gradient budget: at most ONE gradient on the entire page**, and only in a legal home — the three the doctrine allows are (1) inside illustration/photographic artwork, a legibility scrim laid *on an image* included, (2) one editorial accent word in a display heading, (3) a narrow gradient a codified brand language file already specifies. Chart marks are a convention rather than a home: an area fill under a line at ≤15% opacity in the series hue is fine. Never a full-bleed "brand wash" behind a hero value, never behind text on a plain surface, never as a section background.
