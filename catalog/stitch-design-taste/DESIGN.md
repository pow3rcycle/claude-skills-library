...
## 3. Typography Rules
- **Display:** `Geist`, `Satoshi`, `Cabinet Grotesk`, or `Outfit` — Track-tight (`-0.025em`), controlled fluid scale, weight-driven hierarchy (700–900). Not screaming. Leading compressed (`1.1`). Alternatives forced — `Inter` is BANNED, zero exceptions, every context
...
### Banned Fonts
- `Inter` — banned everywhere, zero exceptions, every context
- Generic serif fonts (`Times New Roman`, `Georgia`, `Garamond`, `Palatino`) — BANNED, and so are the four LLM-default display serifs (`Fraunces`, `Instrument Serif`, `Playfair Display`, `Cormorant Garamond`). If serif is needed for editorial/creative, use only `PP Editorial New`, `Tiempos Headline`, `Canela`, `Domaine Display`, `GT Sectra`, or `Gambarino`. Never use default browser serif stacks. Serif is always BANNED in dashboards or software UIs regardless
...
- **Frequency-Gated Motion:** Motion budget scales with how often the user sees the element — rare events (checkout complete, onboarding finish) earn a real ~600ms moment; daily actions (open a panel, switch a tab) are subtle at 120–200ms; anything seen 100× a day (row hover, input focus) is instant or absent. **No perpetual/infinite loops on product components** — Shimmer is legitimate only as a loading skeleton that stops when data lands. All motion honours `prefers-reduced-motion: reduce`
...
- No generic serif fonts (`Times New Roman`, `Georgia`, `Garamond`) and no LLM-default display serifs (`Fraunces`, `Instrument Serif`, `Playfair Display`, `Cormorant Garamond`) — if serif is needed, use `PP Editorial New`, `Tiempos Headline`, `Canela`, `Domaine Display`, or `GT Sectra`
- No perpetual/infinite loop animations (pulse, float, shimmer) outside a loading skeleton; nothing that ignores `prefers-reduced-motion`
